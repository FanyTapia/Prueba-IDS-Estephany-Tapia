...
def extend_calendar(calendar: pd.DataFrame, until) -> pd.DataFrame:
    ...
    festivos fijos) son deterministas y se calculan. ``is_buen_fin`` e
    ``is_semana_santa`` son de fecha móvil y quedan en ``False`` fuera del
    calendario oficial provisto (con advertencia en el log si el horizonte
    cae en meses donde podrían ocurrir).

    Args:
        calendar: Calendario oficial existente (no se modifica).
        until: Última fecha (inclusive) que debe cubrir el calendario.

    Returns:
        DataFrame con las filas originales más las generadas. Si ``until``
        ya está cubierto, devuelve el calendario intacto.
    """
